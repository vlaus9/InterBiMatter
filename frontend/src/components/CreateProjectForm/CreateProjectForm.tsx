...
const CreateProjectForm: React.FC = () => {
    ...
    return (
        ...
        className='absolute flex justify-center items-center w-full h-full bg-black/30 backdrop-blur-xl'>
            <div className='w-[800px] min-w-[400px] h-[600px] min-h-[200px] bg-(--bg-primary) border-2 border-(--button-group-primary-bg) m-[30px] rounded-[15px]'>
                <div className='flex justify-center items-center h-20'>
                    <h1 className='text-(--text-primary) text-[26px] mb-2.5' style={{ fontWeight: 800 }}>Создание проекта</h1>
                </div>
                <form onSubmit={handleSubmit} className='h-[80%] flex flex-col justify-center items-center'>
                    <div className='mb-5 w-[600px] flex justify-between items-center'>
                        <label htmlFor='nameProject' className='mb-2.5 mt-2.5 text-[18px] text-(--text-primary)'>Название проекта</label>
                        <input id='nameProject' type='text' placeholder='Введите название проекта...' value={nameProject} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNameProject(e.target.value)} required className='w-[300px] h-[45px] p-2 border border-[black] bg-(--button-group-primary-bg) outline-(--button-group-primary-bg) rounded-sm text-(--text-primary) placeholder:text-center placeholder:text-[black] placeholder:opacity-[0.5]'></input>
                    </div>
                    <div className='mb-5 w-[600px] flex justify-between items-center'>
                        <label htmlFor='nameProject' className='mb-2.5 mt-2.5 text-[18px] text-(--text-primary)'>Версия файла модели</label>
                        <input id='nameProject' type='text' placeholder='Введите название версии...' value={version} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setVersion(e.target.value)} required className='w-[300px] h-[45px] p-[8px] border border-[black] bg-(--button-group-primary-bg) outline-(--button-group-primary-bg) rounded-sm text-(--text-primary) placeholder:text-center placeholder:text-[black] placeholder:opacity-[0.5]'></input>
                    </div>
                    <div className='flex justify-between items-center w-[600px] '>
                        <h3 className='text-[18px] text-(--text-primary)'>Файл модели</h3>

                        {selectedFiles.length === 0 ?
                            <>
                                <label htmlFor="fileProject" className='flex rounded-sm px-[15px] ml-[50px] w-[300px] h-[45px] bg-(--button-group-primary-bg) justify-center items-center border cursor-pointer hover:border-[#d0cfcfff] hover:border-2 transition-all'>
